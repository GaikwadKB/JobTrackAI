...
class JobRepositoryImpl @Inject constructor(
    ...
    override suspend fun toggleSaveJob(job: Job): DomainResult<Unit> = try {
        if (job.isSaved) {
            ...
            syncDao.addToQueue(SyncQueueEntity(entityType = "JOB", entityId = job.id, operation = "DELETE"))
        } else {
            jobDao.upsertJobs(listOf(job.toEntity().copy(isSaved = true, syncStatus = SyncStatus.PENDING)))
            syncDao.addToQueue(SyncQueueEntity(entityType = "JOB", entityId = job.id, operation = "INSERT"))
        }
